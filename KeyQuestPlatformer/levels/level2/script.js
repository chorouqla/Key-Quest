...
function animate() {
    ...
    cameraOffset.x = Math.max(0, Math.min(
        player.position.x - canvas.width / 2 + player.width / 2,
        collisionsMap[0].length * blockSize - canvas.width
    ))

    cameraOffset.y = Math.max(0, Math.min(
        player.position.y - canvas.height / 2 + player.height / 2,
        collisionsMap.length * blockSize - canvas.height
    ))
    ...
    if (hasKey && doorPosition) {
        ...
        c.drawImage(
            openDoorImage,
            doorX - cameraOffset.x,
            doorY - cameraOffset.y,
            blockSize * 12,
            blockSize * 12,
